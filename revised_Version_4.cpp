...
int main() {
    ...
    // 2D options array. Rows are just 10 digits, solums are the options
    string options[10][4] = {
        {"A. 2", "B. 4", "C. 9", "D. 1984"},
        {"A. 5", "B. 6", "C. 5.5", "D. 4"},
        {"A. 10", "B. 11", "C. 12", "D. 13"},
    ...
    for (int i = 0; i < 10; i++) {
    score += ask_question(questions[i], options[i], answers[i]);
...
//I really do like the array approach. It is a bit more complex at first, but the code looks so nice.
