...
int ask_question(string question, string options[], char correct_answer) {
    ...
    int correctBool;
    int tries = 3;
    //The while loop is only here to allow multiple attempts after messing up an input (The while loop, breaks, continue, try system, and correctBool variable created by Burke)
    while (true){
...
double outputManager(ifstream& quiz_file, string question,string choices[4],char answer){
    ...
    cout << "Quiz Finished!\n";
    cout << "Your score: " << (score/10) * 100 << "%\n"; //Score precentage added by judson
    quiz_file.close();
    return (score/10) * 100;
}
...
int main() {
    ...
    string fileInput;
    int highScore = 0;
    // Menu system added by Judson. It has two options: Input a file or exit the program.
    do {
    ...
    //Prints after the quiz is done (added by Judson)
    cout<<"Your highscore is "<< highScore <<"%.\n\n"; //highscore added by Judson
    } while(fileInput != "0");
